...
public class GetUnassignedShiftGroupsEndpoint : ICarterModule
{
    public void AddRoutes(IEndpointRouteBuilder app)
    {
        ...
        {
            logger.LogInformation("GET /api/shifts/unassigned-groups - Getting unassigned shift groups for Manager {ManagerId}, ContractId={ContractId}",
                managerId, contractId?.ToString() ?? "ALL");
            ...
            if (!result.Success)
            {
                logger.LogWarning("Failed to get unassigned shift groups: {Error}", result.ErrorMessage);
            ...
            logger.LogInformation("Retrieved {GroupCount} unassigned shift groups ({TotalShifts} total shifts) for Manager {ManagerId}, ContractId={ContractId}",
                result.ShiftGroups.Count, totalUnassigned, managerId, contractId?.ToString() ?? "ALL");
        ...
        .AddStandardGetDocumentation<object>(
            tag: "Shifts",
            name: "GetUnassignedShiftGroups",
            summary: "Get unassigned shift groups by manager",
            requiresAuth: false,
