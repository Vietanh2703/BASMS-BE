        ...
        try
        {
            ...
            await connection.ExecuteAsync(@"
                CREATE TABLE IF NOT EXISTS `contracts` (
                    ...
                    `Notes` TEXT NULL,
                    `MonthlyWage` DECIMAL(15,2) NULL COMMENT 'Tiền công cơ bản (VNĐ/tháng) cho working_contract',
                    `MonthlyWageInWords` TEXT NULL COMMENT 'Số tiền bằng chữ: Bảy triệu năm trăm nghìn đồng chẵn',
                    `CertificationLevel` VARCHAR(10) NULL COMMENT 'Hạng chứng chỉ: I, II, III... cho working_contract',
                    `JobTitle` VARCHAR(255) NULL COMMENT 'Chức danh: Bảo vệ thường, Trưởng ca...',
                    `IsDeleted` BOOLEAN NOT NULL DEFAULT FALSE,
                    `CreatedAt` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
                    `UpdatedAt` DATETIME NULL ON UPDATE CURRENT_TIMESTAMP,
                    `CreatedBy` CHAR(36) NULL,
