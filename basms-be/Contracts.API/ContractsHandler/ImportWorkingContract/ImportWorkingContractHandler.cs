...
internal class ImportWorkingContractHandler(
...
{
    public async Task<ImportWorkingContractResult> Handle(
        ImportWorkingContractCommand request,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (signatureCount < 1)
                return new ImportWorkingContractResult
                {
                    Success = false,
                    ErrorMessage =
                        $"Document must have at least 1 signature (Employee - Party B). Found: {signatureCount}"
                };
        ...
        {
            logger.LogError(ex, "Failed to import working contract");
            return new ImportWorkingContractResult
            {
                Success = false,
