...
try
{
    Console.WriteLine("Loading Firebase credentials from appsettings.json (FIREBASE_CONFIG)");

    // Đọc FIREBASE_CONFIG từ appsettings.json
    var firebaseConfig = builder.Configuration.GetSection("FIREBASE_CONFIG");

    if (!firebaseConfig.Exists())
    {
        throw new Exception("FIREBASE_CONFIG section not found in appsettings.json");
    }

    // Convert configuration section thành Dictionary để serialize
    var configDict = new Dictionary<string, string>();
    foreach (var child in firebaseConfig.GetChildren())
    {
        configDict[child.Key] = child.Value ?? string.Empty;
    }

    // Serialize thành JSON string
    var firebaseConfigJson = System.Text.Json.JsonSerializer.Serialize(configDict);

    Console.WriteLine($"Firebase config JSON length: {firebaseConfigJson.Length} characters");
    ...
    Console.WriteLine("✅ Firebase initialized successfully from FIREBASE_CONFIG");
}
catch (Exception ex)
{
